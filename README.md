...
```
  ★ principles (human seed)
    ...
    doc → wiki ── wiki_sync (deterministic bash) ──▶ wiki pages
                                        │
                                        ▼
    wiki E2E verification ── wiki_e2e (bash, after wiki-sync success) ──▶ signal back into the loop
                                        │
                                        ▼
...
7. **Wiki E2E verification** — [`wiki_e2e`](wiki_e2e.md) runs (pure bash) after every successful `wiki_sync`, checking page existence, link integrity, the `.md`-extension adapter, and EN/KO pair completeness. Failures surface as a red CI status on `main`.
...
- ✓ [`wiki_e2e`](wiki_e2e.md) — running (pure bash; triggered by `workflow_run` after `wiki-sync` success + weekly schedule).
