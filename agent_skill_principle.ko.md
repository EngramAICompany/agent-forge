...
3. **composition**: 모듈을 파이프라인으로 연결. 각 모듈은 `in / out / event / failure`를 명시. 기존 task 들을 workflow 로 조립하는 운영 규칙 — 그리고 *언제 새 task 를 쓰지 말아야 하는지* — 은 [Workflow 조립 원칙](workflow_principle.ko.md) 에.
