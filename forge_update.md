...
on ko_sync_completed(sha, verdict, pr?)           → record  K (sha, verdict, pr?)
on wiki_registry_sync_completed(sha, verdict, pr?) → record  R (sha, verdict, pr?)
on forge_pr_approved(pr, head_sha)                → annotate drift_prs entry (informational)

per sha (when all six legs recorded OR W elapsed):
    converged ⇔ W1 ∈ {pushed, skip} ∧ W2 = pass
              ∧ S = skip ∧ C = skip ∧ K = skip ∧ R = skip
    pending   ⇔ ¬converged ∧ ∀ leg ∈ {skip, pushed, pass, pr, escalate}
                  → drift_prs = [pr_url for any leg = pr]
    stale     ⇔ ∃ leg ∈ {failure} ∨ W elapsed with missing leg
                  → missing = legs not in {skip, pushed, pass, pr, escalate, failure}
...
- **event**: consume `wiki_sync_completed`, `wiki_e2e_completed`, `spec_sync_completed`, `ci_spec_sync_completed`, `ko_sync_completed`, `wiki_registry_sync_completed`, `forge_pr_approved`; emit `forge_update_converged | forge_update_pending`. Direct primitive calls are forbidden (composition principle).
