[← Home](Home.md) · [상위 원칙](agent_skill_principle.md) · [ux_agent](ux_agent.md) · [test_agent](test_agent.md) · [ci_trigger](ci_trigger.md) · [pipeline](UX_E2E_CI_plan.md)

# 임의 task 위임 원칙

[에이전트·스킬셋 작성 원칙](agent_skill_principle.md)의 일반화. 특정 도메인(UX/test/CI)에 한정되지 않는 *임의의 task*를 에이전트에게 위임할 때 따르는 규칙.
...
task는 in/out/event를 매개로 다른 task에 연결된다. 다른 task를 직접 호출하지 말고 *이벤트로만* 연결한다. (자세한 의미: [Actor model · pub/sub](agent_skill_principle.md)의 영향. 발신자는 수신자의 존재·구현·상태를 모른다.)
...
변경 신호 없으면 실행하지 않는다. 무효 호출률 = (변경 없음 종료) / (전체 호출). [ci_trigger](ci_trigger.md) 참조.
